...
def get_dataset(dir_paths, batch_size, num_bands=12, shuffle_buffer=200000, crop_season_only=False,
    ...
    def batch_by_fop_old(pds):
        ...
        for d in pds:
            print(d)
        # create fields that could not be batched ----------------------------------------------------------------------
    ...
    pds = tf.map_fn(stack_fields, tf.range(tf.unique))
...
def stack_hexes(row):
    ...
    def num_of_samples_per_hex(idx):
        ret = tf.gather(row['band_vals'], tf.reshape(tf.where(uniques.idx == idx)[0:predict_on_N_imgs], [-1]))
        return ret
