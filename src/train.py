...
top_level_dir_path = 'D:\hexes_with_pixels_s2'
lowest_dirs = list()
for root,dirs,files in os.walk(top_level_dir_path):
    if not dirs:
        lowest_dirs.append(root)

val_files   = [x for x in lowest_dirs if '834' in x]
test_files  = [x for x in lowest_dirs if '831' in x]
train_files = [x for x in lowest_dirs if '832' in x]
