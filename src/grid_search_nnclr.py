...
for i in list(ParameterGrid(parameters)):
    ...
    for index, row in df.iterrows():
        ...
        print('{0:.2f}'.format(pct_complete) + '%', end='')
        print('', end='\r')
        count = count + 1
    ...
    class RandomAugmentSatelliteBands(layers.Layer):
        ...
        def call(self, images, training=True):
            if training:
                ...
                jitter_matrices = tf.concat(
                    [jitter_matrices, jitter_matrices, jitter_matrices, jitter_matrices, jitter_matrices,
                     jitter_matrices, jitter_matrices, jitter_matrices, jitter_matrices, jitter_matrices,
                     jitter_matrices, jitter_matrices], axis=2)
    ...
    pretraining = "{:.2f}%".format(max(pretrain_history.history["val_p_acc"]) * 100)
    ...
    with open('results.txt', 'a') as file:
        file.writelines('NNCLR,' + str(count) + ',' + parameters_str + baseline + ',' + pretraining + ',' + finetuning + '\n')
