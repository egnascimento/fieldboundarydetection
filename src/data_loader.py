...
band_values = tf.constant(['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A', 'B09', 'B11', 'B12'])
number_of_bands = tf.size(band_values)
...
def get_dataset(dir_paths, batch_size, num_bands=12, shuffle_buffer=200000, crop_season_only=False,
    ...
    def parquet_ds(h3_level_dir):
        ...
        def normalize_bands(row):
            row['band_vals'] = (row['band_vals'] - mean_band_vals) / std_band_vals
            return row
        pds = pds.map(normalize_bands)

        return pds

    ds = ds.flat_map(parquet_ds)
    ...
    def stack_fields(row):
        ...
        def stack_hexes(field_idx):
            ...
            fields = tf.gather(row['FIELD_OPERATION_GUID'], tf.reshape(tf.where(unique_fields.idx == field_idx), [-1]))
            bands_from_field = tf.gather(row['band_vals'], tf.reshape(tf.where(unique_fields.idx == field_idx), [-1]))
            ...
            def num_of_samples_per_hex(idx):
                ...
                pad_to_add = tf.clip_by_value(pad_to - tf.shape(bands_from_hexes)[0], clip_value_min=0, clip_value_max=pad_to)
                ...
                zeros_len = tf.clip_by_value(pad_to - tf.shape(hexes), clip_value_min=0, clip_value_max=pad_to)
                hexes = tf.concat([hexes, tf.zeros((zeros_len), dtype=tf.string)], axis=0)
                hexes = hexes[0]

                bands_from_hexes = bands_from_hexes[0:5]

                return hexes,bands_from_hexes

            if debug:
                num_of_samples_per_hex(0)
            band_vals = tf.map_fn(num_of_samples_per_hex, tf.range(tf.size(unique_hexes.y)), fn_output_signature=(tf.string,tf.float64))
            ...
            #padding
            pad_to = 5000
            pad_to_add = tf.clip_by_value(pad_to - tf.size(hexes), clip_value_min=0, clip_value_max=pad_to)
            hexes = tf.concat([hexes, tf.zeros((pad_to_add), dtype=tf.string)], axis=0)
            paddings = tf.concat(([[0,pad_to_add]], [[0,0]], [[0,0]]), axis=0)
            band_vals = tf.pad(band_vals, paddings, mode='CONSTANT', constant_values=0)

            hexes = hexes[0:500]
            band_vals = band_vals[0:500]
        ...
        if debug:
            stack_hexes(0)
        band_vals = tf.map_fn(stack_hexes, tf.range(tf.size(unique_fields.y)), fn_output_signature=(tf.string, tf.string, tf.float64))
