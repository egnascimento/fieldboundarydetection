...
\hyphenation{exis-ten-tes di-fe-ren-tes}
...
% and how it is crucial to understand how we can improve it in further applications.
...
%To overcome this limitation, we propose applying contrastive learning, a method capable of achieving deep learning equivalent results requiring a limited amount of labeled data. Although contrastive learning on agricultural images has been explored successfully~\cite{guldenring2021,jansel2021}, this work proposes, in an unprecedented way, leveraging similar approaches for the automatic detection of agricultural fields. We hypothesize that applying a self-supervised or semi-supervised method should overcome the limitations of obtaining sufficient training data~\citep{yang2020}. Moreover, requiring a smaller amount of labeled samples gives us better control over its quality. \add{And while this work proposes contrastive learning to detect similarity of connected pixels, the delineation of crop fields is implemented using graph-based segmentation~\citep{felzenszwalb2004}, an approach where components of an image are connected by an edge whose weight is proportional to the similarity between them.}

\add{To overcome this limitation, we propose applying contrastive learning to detect the similarity of connected pixels to delineate crop fields using graph-based segmentation, i.e., components of an image are connected by an edge whose weight is proportional to the similarity between them~\citep{felzenszwalb2004}. Contrastive learning is a method capable of achieving deep learning equivalent results requiring a limited amount of labeled data. Although it has been successfully applied to agricultural images~\cite{guldenring2021,jansel2021}, this work proposes, in an unprecedented way, leveraging similar approaches for the automatic detection of agricultural fields. We hypothesize that using a self-supervised or semi-supervised method should overcome the limitations of obtaining sufficient training data~\citep{yang2020}. Additionally, requiring a smaller amount of labeled samples gives us better control over its quality.}
...
%Our hypothesis is that state-of-the-art methods with self-supervised learning are capable of automatically detecting field boundaries requiring a reduced amount of labeled data with good quality. Therefore, contrarily to deep learning, its application in real scenarios is feasible.
...
Finally, the main objective of this project is to evaluate if contrastive learning combined with graph-based segmentation is a promising alternative to established deep learning models, able to detect crop fields automatically but requiring few labeled training samples. %Therefore, our main objective is to train a model capable of classify unforeseen images from remote sensing whose pixels should be labeled as ``crop field'' or ``not a crop field''.





%The objective of this research project is to evaluate the results of employing a state-of-the-art self-supervised method to automatically detect border of crop fields without the need of large amounts of labeled training samples. In that sense, contrastive learning is a promising alternative that has recently demonstrated excellent results, some of them compared to deep learning methods. %Informar que vai usar contrastive learning.
%The main objective of this project is to evaluate if contrastive learning combined with graph-based segmentation is a promising alternative to established deep learning models, able to automatically detect crop fields but requiring few labeled training samples. Therefore, our objective is to train a model capable of classify unforeseen images from remote sensing whose pixels should be labeled as ``crop field'' or ``not a crop field''. %In that sense, we aim to confirm if contrastive learning can be a promising alternative to deep learning methods.

% PROPOSTA
%\section{Crop field detection using contrastive learning and graph-based segmentation}\label{section:constrastive-learning}
...
Once images and their respective labels are properly organized in the dataset, we train the model using \add{contrastive learning. The similarity of the output vectors $\tilde{\boldsymbol{z}_i}$ will be measured using cosine similarity followed by the calculation of the probability of samples to be part of the same class using softmax represented in the Equation ~\ref{eqn:softmax}.
\begin{equation}
softmax=\frac{\exp{\left(\boldsymbol{z}_i\cdot\boldsymbol{z}_j\right)}}{\sum_{k=1}^{2N}\mathbf{1}_{i\neq k}\cdot\exp{\left(\boldsymbol{z}_i\cdot\boldsymbol{z}_k\right)}}
\label{eqn:softmax}
\end{equation}
This information will be used to create a graph where the nodes are the pixels and the edge weights are the probability of the two connected nodes to be part of the same class. Graph-based~\citep{felzenszwalb2004} segmentation will be used to distinguish field and non-field pixels in the map using graph cuts~\citep{veksler2001}. The lowest probabilities identify the field boundary or the line that separates two different groups of classes.
as it is highlighted in bold red in Figure~\ref{figure:hexgraph}.
\begin{figure}[ht]
\centering
\includegraphics[height=6cm]{Projeto de Qualificação - Eduardo/res/hexgraph.png}
\caption{\label{figure:hexgraph}Graph-based segmentation with contrastive learning probabilities\protect}
\end{figure}
...
% Describe how we plan the implementation
Once remote sensing images and their respective labels are properly organized in the dataset, we train the model using \add{contrastive learning. The similarity of the output vectors $\tilde{\boldsymbol{z}_i}$ will be measured using cosine similarity followed by the calculation of the probability of samples to be part of the same class using softmax represented in the Equation ~\ref{eqn:softmax}.
\begin{equation}
softmax=\frac{\exp{\left(\boldsymbol{z}_i\cdot\boldsymbol{z}_j\right)}}{\sum_{k=1}^{2N}\mathbf{1}_{i\neq k}\cdot\exp{\left(\boldsymbol{z}_i\cdot\boldsymbol{z}_k\right)}}
\label{eqn:softmax}
\end{equation}
...
% e comparar os resultados com o método da artes
...
\begin{itemize}
    ...
    \item Matthew's correlation coefficient (MCC): an effective solution \add{for} imbalanced datasets.
\end{itemize}
